class DebugRenderer
  getter config : DebugConfig

  @current_y : Int32 = 0

  def initialize(@config : DebugConfig = DebugConfig.new)
  end

  def render(items_by_category : Hash(String, Array({DebugRegistry::ItemDefinition, String?}))) : Void
  ...
  private def calculate_category_height(items : Array({DebugRegistry::ItemDefinition, String?})) : Int32
  ...
  private def draw_item(definition : DebugRegistry::ItemDefinition, value : String?) : Void
